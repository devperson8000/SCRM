...
function handleCss(
  ...
  // Uses [uU][rR][lL] instead of the (?i:url) inline-flag group: that syntax
  // (regex modifiers, TC39 stage 4) isn't supported by Rollup/acorn's parser
  // yet, which broke production bundling wherever this file gets pulled in
  // directly rather than through the prebuilt dist bundle.
  const urlRegex =
    /[uU][rR][lL]\((?:\s*"((?:\\.|[^"])+)"\s*|\s*'((?:\\.|[^'])+)'\s*|((?!\s*['"])(?!\s*\))(?:\\.|[^)])+?))\)/gm;
  const Atruleregex =
    /@import\s+([uU][rR][lL]\s*?\(.{0,9999}?\)|['"].{0,9999}?['"]|.{0,9999}?)($|\s|;)/gm;
  css = String(css);
  css = css.replace(
    urlRegex,
    (
