...
export class Controller {
  ...
  private cookiePersistenceEnabled = true;
  ...
  constructor(public init: ControllerInit) {
    ...
    navigator.serviceWorker.addEventListener("message", (e) => {
      if (
        ...
        if (typeof payload.id === "string") {
          this.currentServiceWorker().postMessage({
            $sw$setCookieDone: {
              id: payload.id,
            },
          });
      ...
      if (e.data.$controller$swrevive) {
        const knownIds = (e.data.$controller$swrevive as { knownIds?: unknown })
          .knownIds;
        if (Array.isArray(knownIds)) {
          // The worker enumerated the controllers it is actually holding a
          // port for. Re-attaching when ours is among them would close a live
          // channel and strand every request in flight on it — which the
          // repeated broadcasts from a worker waiting on some *other* tab
          // would otherwise make us do once a second.
          if (knownIds.includes(this.id)) return;
        } else if (this.guardServiceWorkerRevive) {
          // A worker bundle old enough not to say what it holds. Fall back to
          // the timing heuristic: if we just spawned it, it sends this even
          // though it's not actually dead.
    ...
    // The browser terminates an idle service worker within about half a minute
    // of a tab being backgrounded, and it takes this controller's MessagePort
    // with it. Nothing on the page notices, so the failure surfaced on the
    // user's first click after coming back: that click restarted the worker,
    // and the request it made was handled by an instance that had never heard
    // of us. Checking on the way back in restarts the worker while the user is
    // still looking at the page — if it has forgotten us it says so, and the
    // port is rebuilt before anything is clicked.
    if (typeof document !== "undefined") {
      document.addEventListener("visibilitychange", this.onPageRevisited);
      // Restoring from the back/forward cache resumes a page that may have
      // been frozen for hours without ever firing visibilitychange.
      window.addEventListener("pageshow", this.onPageRevisited);
    }
  }

  /**
   * The service worker to talk to right now. The `ServiceWorker` captured at
   * construction survives the worker being stopped and restarted, but not the
   * registration being updated: the object then refers to a redundant worker,
   * and postMessage to a redundant worker is dropped without an error, leaving
   * this controller holding a port nothing is listening on.
   */
  private currentServiceWorker(): ServiceWorker {
    const active = navigator.serviceWorker.controller;
    if (active) {
      this.serviceWorkerController = active;
    }

    return this.serviceWorkerController;
  }

  private onPageRevisited = () => {
    if (document.hidden) return;
    // Delivering this restarts a stopped worker, which is the point: either it
    // still holds our port and says so, or it doesn't and the reply is what
    // tells us to hand over a new one.
    this.currentServiceWorker().postMessage({
      $sw$ping: { id: this.id },
    });
  };
  ...
  private setupMessagePort() {
    const serviceworker = this.currentServiceWorker();
    ...
    serviceworker.postMessage(
      {
        $controller$init: {
          prefix: this.prefix,
          id: this.id,
          // So a worker that restarts with an empty routing table can still
          // tell a proxied URL from one belonging to the app itself.
          prefixRoot: this.config.prefix,
        },
      },
      [channel.port2],
    );
  ...
  setCookiePersistence(enabled: boolean) {
    this.cookiePersistenceEnabled = enabled;
  }
