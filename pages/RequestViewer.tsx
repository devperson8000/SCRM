...
RequestCard.style = css`
  ...
  .request-row:hover {
    border-color: var(--line-strong, rgba(190, 230, 219, 0.2));
    background: rgba(255, 255, 255, 0.05);
  }
  .request-row.selected {
    border-color: var(--accent-border, rgba(0, 255, 136, 0.35));
    box-shadow: inset 3px 0 var(--accent, #00ff88);
  ...
  .request-method {
    ...
    border: 1px solid var(--accent-border, rgba(0, 255, 136, 0.28));
...
const RequestViewer: Component<
  ...
  },
  {
    ...
    pendingRequests?: RequestEntry[] | null;
...
> = function () {
  ...
  // Blob URLs minted for image/video previews are held by the browser until
  // they are explicitly revoked. Entries fall off the end of the log
  // constantly (maxRequests defaults to 200) and every dropped one used to
  // leave its blob pinned in memory for the life of the page, so browsing an
  // image-heavy site with the viewer open grew without bound.
  const revokeEntryMedia = (entry: RequestEntry) => {
    if (entry.responseBodyMediaUrl) {
      URL.revokeObjectURL(entry.responseBodyMediaUrl);
    }
    if (entry.responseBodyMediaUrlPre) {
      URL.revokeObjectURL(entry.responseBodyMediaUrlPre);
    }
  };

  // Batched so a burst of requests repaints the list once per frame rather
  // than once per hook.
  //
  // The updaters are applied *eagerly* to a pending snapshot, and only the
  // result is handed to the state on the next frame. Queuing the updaters
  // themselves (by composing each new one around the last, as this used to)
  // built an ever-deeper chain of closures whenever the frame never came --
  // which is exactly what happens in a background tab, where requestAnimationFrame
  // does not fire at all. A page left proxying in another tab accumulated one
  // closure per captured request and then unwound all of them in a single
  // nested call on return, deep enough to risk a stack overflow.
  const updateRequests = (
    updater: (prev: RequestEntry[]) => RequestEntry[],
  ) => {
    this.pendingRequests = updater(
      this.pendingRequests ?? requestsState.requests,
    );
    ...
    this.pendingRequestsFlush = requestAnimationFrame(() => {
      this.pendingRequestsFlush = undefined;
      const next = this.pendingRequests;
      this.pendingRequests = null;
      if (!next) return;

      captureListAnchor();
      requestsState.requests = next;
      restoreListAnchor();
    });
  };
  ...
  const clear = () => {
    // Cancel any queued flush first: without this, a batch captured moments
    // before the click was still pending and repopulated the list right
    // after it was emptied.
    if (this.pendingRequestsFlush !== undefined) {
      cancelAnimationFrame(this.pendingRequestsFlush);
      this.pendingRequestsFlush = undefined;
    }
    for (const entry of this.pendingRequests ?? requestsState.requests) {
      revokeEntryMedia(entry);
    }
    this.pendingRequests = null;
  ...
  const initPlugin = (frame: Frame | null | undefined) => {
    // browserState.frame starts out null and is only filled in once the first
    // browser tab has mounted. Opening the Requests tab before that (or after
    // a failed mount) used to run straight into `frame.hooks` on null --
    // which threw *after* pluginReady had already been set, so the viewer
    // never taps the hooks again and silently captures nothing for the rest
    // of the session. Waiting for the frame is enough: this signal re-runs
    // when it arrives.
    ...
    plugin.tap(frame.hooks.fetch.request, (context, props) => {
      ...
      updateRequests((prev) => {
        ...
        const overflow = next.length - demoSettingsStore.maxRequests;
        if (overflow > 0) {
          for (const dropped of next.slice(0, overflow)) {
            revokeEntryMedia(dropped);
          }
        }
        if (!this.selectedId) {
...
RequestViewer.style = css`
  :scope {
    ...
    flex-direction: column;
    min-height: 0;
    background: var(--surface-0, #05090b);
  }
  .tab-action {
  ...
  .requests-search:focus {
    ...
    box-shadow: 0 0 0 3px var(--accent-dim, rgba(0, 255, 136, 0.1));
  }
  .requests-toggle {
    display: inline-flex;
    align-items: center;
  ...
  .requests-empty {
    padding: 1em;
    border: 1px dashed var(--line-strong, rgba(190, 230, 219, 0.2));
    border-radius: 8px;
    text-align: center;
    color: var(--text-3, #71847f);
  }
  ...
  .detail-section {
    border-top: 1px solid var(--line, rgba(190, 230, 219, 0.1));
    padding: 0.65em 0;
  }
  ...
  .detail-block {
    ...
    border: 1px solid var(--line, rgba(190, 230, 219, 0.1));
    border-radius: 8px;
    padding: 0.6em 0.7em;
    margin-bottom: 0.6em;
  }
